...
def start_mpi_exec(procs, slaves, program):
    ...
    t = threading.Thread(target=print_output, args=([p]))
    t.start()

    return proxy_args


def finalize_slaves(callbacks):
    time.sleep(1)
    logging.info("Finalize slaves")
    hosts = []
    for slave in callbacks:
        hosts.append(slave[0])
    proxy_args = start_mpi_exec(total_procs, hosts, mpi_program)
    ...
    def statusUpdate(self, driver, update):
        if update.state == mesos_pb2.TASK_FAILED \
            or update.state == mesos_pb2.TASK_KILLED \
                or update.state == mesos_pb2.TASK_LOST:
...
if __name__ == "__main__":
    ...
    parser.add_option("-N", "--nodes", help="number of nodes to run processes (default 1)",
                      dest="nodes", type="int", default=1)
    parser.add_option("-n", "--num", help="total number of MPI processes (default 1)",
                      dest="procs", type="int", default=1)
    parser.add_option("-c", "--cpus-per-task", help="number of cores per MPI process (default 1)",
